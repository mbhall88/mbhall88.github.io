...
description: "Depth normalisation, filtering internal matches, and the performance work behind LRGE 1.0.0."
...
uncovered a bug that had been affecting every PacBio estimate made through the command line.

{{< figure src="graphical-abstract-technical.png" alt="LRGE method: minimizer counts detect uneven depth and guide sampling of target and query reads. Minimap2 counts read-to-read overlaps, giving per-read genome size estimates that are summarised by a median and interval. Version 1 adds conditional depth normalisation, fixes platform selection, fits intervals per platform, and improves read processing with packed DNA, parallel retention scoring and a capped buffer." caption="How LRGE works, with v1 changes marked in amber. Both read sets are sampled from the input; their default sizes are shown. The performance changes below apply to depth normalisation and read selection." >}}
...
(SRR12247681) had two plasmids at 50,000× and 160,000× depth. In a *Pandoraea fibrosis* dataset
(SRR9733840), the rRNA genes reached 100,000× depth.

With such uneven depth, a small part of the genome can supply most of the reads. Random sampling
can therefore select mostly reads from the amplified sequence, which overlap each other frequently.
Their per-read genome size estimates are small, and if they dominate the sample, so is the median
([issue #29][i29]).
...
pass that counts input reads, it computes minimizers for about 1% of them, sampling more on small
inputs. A count-min sketch stores approximate minimizer frequencies. The detector compares the
99.9th percentile count with the median; a ratio of at least 16 triggers normalisation. Otherwise,
LRGE uses its ordinary random sampling path.
...
combinations across the full bacterial benchmark and additional low-depth datasets. A broad range
performed similarly; the defaults of 16 and 2 balance correcting skew against unnecessarily
changing already reasonable estimates. The [parameter experiments][r36] give the full comparison.
...
initial normalisation implementation repeatedly packed overlapping k-mers from ASCII, in both
forward and reverse-complement orientations. LRGE now packs each ACGT stretch into two bits per
...
Building the depth profile and scoring reads also use multiple threads. Scoring can happen in
...
size from 13 to 6, but increased average error overall.
...
For example, compare `lrge -s 42 -vv reads.fq.gz` with `lrge -s 42 -vv -F reads.fq.gz`, using
`-P pb` in both commands for PacBio reads. Keeping the seed fixed makes the comparison use the same
sampled reads. The log reports whether the filter engaged; if the share doesn't exceed 0.8, a bare
`-F` leaves the estimate unchanged. The [filter experiments][r36] explain the threshold tradeoffs.

## The platform flag that did nothing

LRGE also reports an interval based on the spread of its per-read estimates. Normalisation changes
...
Passing the platform from the CLI through to the overlap code now selects `ava-pb` for PacBio,
...
bug fixed, I recalibrated the interval bounds on the bacterial benchmark. Nanopore and PacBio needed
different percentiles, so the defaults now follow `-P`.
...
Here is 0.3.0 against 1.0.0 on the paper's 3,370 bacterial datasets.[^comparison] Absolute relative
...
truth rises from 58.6% to 62.9%. Raven, which assembles the reads to estimate genome size, remains more
accurate, but at the cost of runtime and memory usage, as shown in the [paper's resource
comparison][resources].

PacBio is still the weaker platform. Getting about a quarter of its datasets within 10% of the truth
...
The [installation instructions][install] cover those, Cargo, and containers.

This is a breaking release because the results change. If you're reproducing the published
benchmark, pin the 0.3.x series; if you're reproducing another analysis, keep its exact version and
options.
